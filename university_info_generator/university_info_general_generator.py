...
from university_info_generator.fetcher._gpt_method import get_university_name_from_gpt, get_value_and_reference_from_gpt
...
class UniversityInfoGenerator:
    ...
    def get_university_basic_info(self, param: str, param_type: BasicInfoType) -> Dict[str, str]:
        ...
        if gpt_dict_key in self.gpt_cache_dict:
            return self.gpt_cache_dict[gpt_dict_key]
        json_str = get_university_name_from_gpt(param)
    ...
    def get_university_info(self, university_name: str) -> University:
        ...
        if university_name not in self.university_basic_info_dict:
            result = self.get_university_basic_info(param=university_name, param_type=BasicInfoType.UNIVERSITY_NAME)
            for key, value in result.items():
                if value != "":
                    university_json[key] = value
                    if key == "university_name":
                        university_name = value
            reference += result.get("website", "") + " " + result.get("wikipedia", "")

        for attribute_name in self.attribute_dict:
            if (
                gpt_dict_key := str(((university_name, attribute_name), GPTMethodType.ATTRIBUTE_INFO))
            ) in self.gpt_cache_dict:
                university_json[attribute_name] = self.gpt_cache_dict[gpt_dict_key]
                continue
            # seperate the tuition logic to use crawl directly
            if attribute_name in ("domestic_student_tuition", "international_student_tuition"):
                result = self.tuition_crawl.fetch_tuition(university_name)
                if isinstance(result, dict):
                    university_json["domestic_student_tuition"] = result.get("domestic_student_tuition", "N/A")
                    university_json["international_student_tuition"] = result.get(
                        "international_student_tuition", "N/A"
                    )
                else:
                    # Log error or set default values
                    print(f"Unexpected result type from fetch_tuition: {result}")
            ...
            generated_attr, _ = get_value_and_reference_from_gpt(
            ...
            self.gpt_cache_dict.update({gpt_dict_key: generated_attr})
